...
{
	ASTATEMACHINE_PRINTF("stateMachine %s has %llu states\r\n", sm->name, cvector_length(sm->states));
	for (int i = 0; i < cvector_length(sm->states); i++)
	{
...
void aStateMachine_addTransition1(aStateMachine* sm, aState* from, aState* to, bool(*condition)(aStateMachine* statemachine))
{
	...
	for (int i = 0; i < cvector_length(sm->states); i++)
	{
		if ( ((aState*)cvector_at(sm->states, i))->name == from->name) {		////////两个指针指向的是同一块内存，但它们两个自己是不同的，不能直接比较
			//找到from状态
...
void aStateMachine_start(aStateMachine* sm, aState* s)
{
	if (sm->started)
	{
		ASTATEMACHINE_PRINTF("stateMachine %s has already started\r\n", sm->name);
		return;
	}
	sm->started = true;
	sm->current = s;
...
void aStateMachine_update(aStateMachine* sm)
{
	...
	if (!sm->started)
	{
		ASTATEMACHINE_PRINTF("stateMachine %s has not started\r\n", sm->name);
		return;
	}
#endif
	if (sm->finished)
	{
		...
		ASTATEMACHINE_PRINTF("stateMachine %s has finished\r\n", sm->name);
#endif
		return;
	}
	if (sm->current != NULL)
	{
		...
		for (int i = 0; i < cvector_length(sm->current->transitions); i++)
		{
			...
			if (t->trigger)
			{
				...
				ASTATEMACHINE_PRINTF("update debug: transition %s to %s not triggered", aState_getName(sm->current), aState_getName(sm->current));
			}
#endif
		}
